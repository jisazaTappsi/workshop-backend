...
from create_db_and_table import Hero, engine
...
@app.get("/")
def hello_world():
    return {"message": "Hello some World"}


@app.post("/")
async def add_new_hero(request: Request):
    # let's add a bug and solve it with the debugger
    body = await request.json()
    hero = Hero(name=body['name'], secret_name=body['secret_name'], age=int(body['age']))

    with Session(engine) as session:
        session.add(hero)
        session.commit()

    return {"message": "added something to DB successfully"}
